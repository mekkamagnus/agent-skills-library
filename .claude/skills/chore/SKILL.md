...
hooks:
  PreToolUse:
    - matcher: "Write"
      hooks:
        - type: command
          command: "python3 /Users/mekael/.zcode/v2/acp-config/claude/d1e7c6d3d969/skills/chore/validate_chore_name.py"
---
...
- Create the plan in the `docs/specs/` directory. The filename MUST follow the format `CHORE-##-{slug}.md` where `##` is a sequential number. Run the numbering script to determine the next number:
  ```bash
  python3 ~/.claude/skills/chore/next_chore.py <project-root>/docs/specs <chore-slug>
  ```
  Example: `python3 ~/.claude/skills/chore/next_chore.py ./docs/specs update-deps` → outputs `CHORE-01-update-deps.md`.
